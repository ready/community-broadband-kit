...
export default function customFetch (uri, inputOptions) {
  ...
  // Get selected organization json from localStorage
  const selectedOrganizationJson = window.localStorage.getItem(SELECTED_ORGANIZATION_KEY)

  // Send selected organization id header
  if (selectedOrganizationJson) {
    options.headers.selectedOrganizationId = JSON.parse(selectedOrganizationJson).id
  }
        ...
        refreshingPromise = fetch(address, { ...options, method: 'POST' })
          .then((refresh_token_repsonse) => {
            if (refresh_token_repsonse.ok) {
              ...
              if (pathname !== '/signup') history.push('/')
            }
          })
      }
