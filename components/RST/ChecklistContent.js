...
const ChecklistContent = ({ currentChecklist }) => {
  ...
  // fetch terms data from strapi api
  ...
  const fetchStrapiDefinition = async () => {
    const res = await fetch(
      `https://api.broadband.money/api/broadband-grant-terms?${query}`
    )
    const terms = await res.json()

    setTerm(terms?.data)//?.attributes?.definition
  }
  ...
  const handleOnChange = (field, value) => {
    const data = {...toolkitData, [field]: value}
    setToolkitData(data)
  }
  ...
  const checklist = {
    internet: (
      <div className={styles.checklistQuestion}>
          ...
          <RSTTooltip
            ...
            tooltipText={
              <div className={styles.tooltipText}>
                ...
                <img className={styles.ethernetImage} src='/images/ethernet.png' alt='ethernet image' />
        ...
        {toolkitData?.connectionType?.includes('WiFi') &&
                ...
                <RSTTooltip
                  ...
                  tooltipText={
                    <div className={styles.tooltipText}>
                      ...
                      <img className={styles.ethernetImage} src='/images/router.png' alt='router image' />
                    </div>
                  }
                />
              </div>
