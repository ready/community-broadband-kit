...
async function initSurvey(
  ...
  if (surveyType === 'stand-alone') {
    ...
    questionProgressElement = document.getElementById(
      'individual-questions-answered'
    );
    surveyProgressBarElement = document.getElementById(
      'stand-alone-survey-progress-bar'
    );
    ...
    if (isHomeArray) {
      for (let i = 0; i < isHomeArray.length; i++) {
        if (
          isHomeArray[i].ipAddress === ipAddress &&
...
async function addToAnsweredQs(questions, ipAddress) {
  ...
  if (answeredQsArray === null) {
    localStorage.setItem(
      'answeredQs',
      JSON.stringify([{ ipAddress: ipAddress, answered: questions }])
    );
  }
...
async function updateIsHomeLocalStorage(state) {
  ...
  if (isHomeArray === null) {
    localStorage.setItem(
      'isHome',
      JSON.stringify([{ ipAddress: ipAddressConstant, answered: state }])
    );
  }
...
async function nextQuestion() {
    ...
    if (standAloneSurvey) {
      ...
      if (answers.length === 0) {
        answers = document.querySelectorAll(
          '#stand-alone-survey-answers select'
        );
      }
    } else {
  ...
  questionProgressElement.textContent = questionIndex;
  surveyProgressBarElement.value =
    surveyProgressBarElement.value + progressIncrement;
 ...
 */
async function uploadSurveyData(
  uuid,
  addressLat,
  addressLon,
  addressText,
  ...
  query += `}) {
        ...
        createdAt
        updatedAt
        additionalInternetUse
        address
        addressLat
        addressLon
        attestation
        cellSatisfaction
        challengeProcessAuthorization
        communicationUseRank
        countAttendLocation
        countChildren
        countDevices
        countDisabled
        ...
        countWorkFromHome
        currentCost
        deviceSetupAssistance
        deviceCostAssistance
        deviceSecurityAssistance
        dissatisfactionReasons
        dissatisfactionReasonsMultiple
        downloadTier
        downloadingUploadingAssistance
        educationUseRank
        entertainmentUseRank
        governmentRole
        hasTroublePaying
        hasTroublePayingReason
        householdIncome
        idealAdditionalCost
        idealCost
        idealGigCost
        internetConnectionAvailabilityAssistance
        internetImportance
        internetNavigationAssistance
        internetType
        internetUse
        ipAddress
        ispName
        locationType
        movingCuttingPastingAssistance
        nameOnInternetBill
        onlineFormsAssistance
        otherComments
        otherUseRank
        phoneNumber
        reliabilitySatisfaction
        softwareApplicationAssistance
        speedSatisfaction
        struggleMost
        subscriptionEvidenceFile
        telehealthUseRank
        timeOfSpeedTrouble
        uploadTier
        usesCell
        usesHighestTier
        usesInternetForSchool
        videoCallLag
        websiteCreationAssistance
        workUseRank
        wouldStartBusiness
        wouldUseCredit
    }}`;
  ...
  return fetch(BOSS_PLATFORM_URL, {
    method: 'POST',
    headers: {
